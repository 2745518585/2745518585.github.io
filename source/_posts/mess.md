...
#### [QOJ8526](https://qoj.ac/problem/8526) Polygon II

转化题意为求最大的数不超过其他数之和的概率。记 $n-1$ 个数的和减去剩下的数的差为 $S$，考虑求出 $S<0$ 的概率，在二进制下按位考虑，设 $b_i$ 表示有多少个 $a_j \geq i$，$f_{i,j}$ 确定前 $i-1$ 位，上一位进位为 $j-1$。由于 $b_i$ 个数每个数等概率选择 $0$ 和 $1$，那么对 $S$ 当前位的贡献为 $b_i$ 个 $0/1$ 的和减一，枚举有其中有 $k$ 个 $1$，那么有转移 $\frac{f_{i-1,j} \binom{b_i}{k}}{2^{b_i}} \rightarrow f_{i,\lfloor \frac{j+k}{2} \rfloor}$。

考虑统计答案，设最大的那个数位数为 $i$，显然其他数不能在高于 $i$ 位出现 $1$，那么 $f_{i,0}$ 乘上 $>i$ 的位均为 $0$ 的概率即为答案，加起来即可。

#### [QOJ5175](https://qoj.ac/problem/5175) 翻修道路

记 $f_{i,j,k}$ 表示从 $j$ 出发到达 $i$ 中的关键点修改 $k$ 次后最大距离的最小值，容易 $3^n m^3$ 转移，注意到合并两个状态是对两个数取 $\max$，在第三维双指针即可，$O(2^n m^3 + 3^n m^2)$。

#### [QOJ5098](https://qoj.ac/problem/5098) 第一代图灵机

假设没有修改，容易想到对于每个 $i$ 找到其作为右端点时最小左端点 $g_i$，有 $g_i = \max_{j=1}^i\{pre_j\}$，其中 $pre_j$ 为最大的满足 $a_j=a_k,k<j$ 的 $k$。而前缀最大值这个东西无法简单维护，题解做法为线段树分治维护。

考虑直接用线段树做，求答案时需要满足所有数对应的左端点不小于询问的 $L$，因此考虑线段树上快速求出对于一个节点 $[l,r]$ 仅考虑 $pre_{l \sim r}$ 和任意 $lim$ 限制时的答案。考虑对于一个节点 $[l,r]$ 维护仅考虑 $pre_{l \sim r}$ 限制时 $[mid+1,r]$ 的答案（记为 $s_{l,r}$），假设这个值已经求出来了，考虑处理查询，有：

- 当 $\max_{i=l}^{mid} \{pre_i\} \geq lim$ 时，显然去掉 $lim$ 后右侧的数的限制不会变小，那么右侧答案即为 $s_{l,r}$，递归左侧后取 $\max$ 即可。
- 当 $\max_{i=l}^{mid} \{pre_i\} < lim$ 时，显然左侧所有数的最紧限制均为 $lim$，左侧答案为 $\sum\limits_{i=lim}^{mid} a_i$，递归右侧后取 $\max$ 即可。

发现所有操作都是单侧递归的，复杂度为 $\log^2$。考虑求 $s_{l,r}$，不难发现 $s_{l,r}$ 即为查询右子树 $lim=\max_{i=l}^{mid} \{pre_i\}$ 的答案。

#### [QOJ8670](https://qoj.ac/problem/8670) 独立

考虑求最大独立集的经典 dp：设 $dp_{x,0/1}$ 表示 $x$ 是否选中的答案。感性理解一下 $\max(dp_{x,0},dp_{x,1})-dp_{x,0}$ 即为 $x$ 的贡献，大概就是包含 $x$ 的答案减去去掉 $x$ 的答案，多算的在后面一定会抵消，设 $f_{x,i}$ 表示 $\max(dp_{x,0},dp_{x,1})-dp_{x,0}=i$ 的方案数，答案即为 $f_{x,i} \times i$ 再乘上外面的方案 $m^{n-size_x}$。

由于 $i$ 是值域范围的，考虑拉插，不难证明 $f_{x,i}(i \neq 0)$ 是关于 $i$ 的 $size_x$ 次多项式。考虑转移，相当于是 $\sum_u f_{u,p_u} \rightarrow f_{x,\max(1-\sum_u p_u,0)}$，可以减法卷积得到任意 $m-size_x+1 \sim m$ 后点值平移到 $1 \sim size_x$。注意到式子中的 $\max$，也就是 $<0$ 的全部会贡献到 $0$，可以一次拉插得到 $1 \sim m$ 的答案后用总数减去即可。复杂度 $O(n^2 \log n)$。
